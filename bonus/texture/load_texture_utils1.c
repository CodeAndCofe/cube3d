...
int get_pixel_color(char *text_side, int text_x, int text_y, t_player *player)
{
    ...
    {
        line_len = player->data->door_tex_line_len;
        bpp = player->data->door_tex_bpp;
        width = player->data->door_tex_width;
        height = player->data->door_tex_height;
    }
    else
    {
        line_len = player->data->tex_line_len;
        bpp = player->data->tex_bpp;
        width = player->data->tex_with;
        height = player->data->tex_height;
    }

    color = get_texture_pixel(text_side, text_x, text_y, line_len, bpp);
    return (color);
}
